...
const parameters = {
  ...
  height: 400,
  foodInfluence: 1,
  colors: ["#EBE9E6", "#967849", "#4A226B", "#A6EEC1"],
  foodColor: [1, 0, 0, 1],
  foodShowInfluence: 0.0,
  sensorDistance: 16,
  zoomDistanceRatio: 1,
...
function loadLines() {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (let j = 0; j < linePoints.length; j++) {
      ...
      let pos = myMap.latLngToPixel(linePoint[1], linePoint[0]);
      console.log("point:" + pos);
      mapCanvas.push();
      mapCanvas.fill("red");
      mapCanvas.ellipse(pos.x, pos.y, 2, 2);
      mapCanvas.pop();
    }
  }
}
...
function setupGui() {
  ...
  gui
    .add(parameters, "foodShowInfluence", 0, 1, 0.01)
    .name("Food Show Influence");
...
function setup() {
  ...
  mapCanvas = createGraphics(windowWidth-266, windowHeight-266);
  mapCanvas.parent(canvasContainer);
  mapCanvas.background(0);

  slimeCanvas = createCanvas(windowWidth-266, windowHeight-266, WEBGL);
  slimeCanvas.parent(canvasContainer);
...
function draw() {
  ...
  // texture(scentImage);
  combinedCanvas.rect(
    -combinedCanvas.width * 0.5,
    -combinedCanvas.height * 0.5,
    combinedCanvas.width,
    combinedCanvas.height
  ); // draw the combined results
  // Run simulation
